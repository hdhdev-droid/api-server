...
|------|------|
| `PORT` | 서버 포트 (기본값: 3000) |
| `DB_TYPE` | DB 유형: `POSTGRESQL`, `MYSQL`, `MARIADB`, `MONGODB` 중 하나 |
| `DB_HOST` | DB 호스트 |
| `DB_PORT` | DB 포트 (기본: PostgreSQL 5432, MySQL/MariaDB 3306, MongoDB 27017) |
| `DB_NAME` | DB(데이터베이스/스키마) 이름 |
| `DB_USER` | DB 사용자 |
| `DB_PASSWORD` | DB 비밀번호 |
| `DB_URI` | (선택) MongoDB 연결 문자열. 없으면 위 호스트/포트 등으로 연결 |

로컬에서는 프로젝트 루트에 `.env` 파일을 두거나, `.env.example`을 복사해 `.env`로 만든 뒤 값을 채워 사용할 수 있습니다. (실제 값은 Git에 올리지 마세요.)

**지원 DB**: PostgreSQL, MySQL, MariaDB, MongoDB. **DB 연결이 필수**이며, 변수·메모리 DB는 사용하지 않습니다. `DB_TYPE`과 `DB_HOST`, `DB_NAME` 등이 설정되고 연결에 성공해야 서비스 접속이 가능합니다. DB가 설정되지 않았거나 연결에 실패하면 503 접속 불가 화면이 표시됩니다. 테이블/컬렉션이 없으면 첫 요청 시 자동 생성됩니다.
